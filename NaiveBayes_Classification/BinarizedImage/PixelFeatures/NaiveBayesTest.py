...
class NaiveBayesTest():
        ...
        for yesline in yeslines:                      
            ...
            if x < 25:
                for element in range(0,10):
                    ...
                    temp.append(x)
                    temp.append(element)
                    #Calculate sum of log of each pixel for both yes and no
                    if yesline[element] is '%':                        
                        yessum = yessum + math.log(self.trained.yeslow[tuple(temp)])
                        nosum = nosum + math.log(self.trained.nolow[tuple(temp)])
                        elementlist.append(0)
                    else:                        
                        yessum = yessum + math.log(self.trained.yeshigh[tuple(temp)])
                        nosum = nosum + math.log(self.trained.nohigh[tuple(temp)])
    ...
    def confusionmatrix(self):
        #Nomenclature:TrueLabel,AssignedLabel
        yesyes = 0
        yesno = 0
        nono = 0
        noyes = 0
